...
const FundsController = {
        ...
        await db(process.env.DB_T_USERS)
            ...
            .then(() => {
                db(process.env.DB_T_USERS)
                    ...
                    .then(response => res.status(200).json({ data: response, message: "funding is successful" }))
    ...
    transferFunds: async(req, res) => {
        ...
        //the bcrypt compare:
        if ((sender_acctExists && await bcrypt.compare(String(transaction_pin), sender_acctExists.transaction_pin))) {
                ...
                await db(process.env.DB_T_USERS)
                    ...
                    .then(async() => {
                        db(process.env.DB_T_USERS)
                            .where({ account_no: sender_acct_no })
                            .select("id", "first_name", "last_name", "email", "amount")
                            .then(response => res.status(200).json({ data: response, message: "transaction is successful" }))
                            .catch(err => res.status(404).json(err));
                    })
                    .catch(err => res.status(404).json({ message: "user not found", error: err }));
            })
    ...
    withdraw: async(req, res) => {
        ...
        if (acctExists && await bcrypt.compare(String(transaction_pin), acctExists.transaction_pin)) {
            return res.status(400).json("incorrect pin");
        };
